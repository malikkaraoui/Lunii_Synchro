...
def _sanitize(name: str) -> str:
    """Supprime les accents et caractères non-ASCII pour éviter les bugs SPG."""
...
def _generate_zip(audio_file: Path, out_dir: Path) -> str:
    safe_stem = _sanitize(audio_file.stem)[:50] or "audio"
    safe_name = safe_stem + audio_file.suffix.lower()
    sd = out_dir / safe_stem
    sd.mkdir()
    shutil.copy2(audio_file, sd / safe_name)
    env = os.environ.copy()
