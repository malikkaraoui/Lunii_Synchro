...
- **Décision** : Réécriture Tauri 2.0 — Rust pour fenêtre native + détection USB + inventaire ; Python (`boite-bridge.py`) gardé comme sidecar uniquement pour la crypto boîte à histoires (dépendance `StoryBox.QT`)
...
- **À revalider si** : `StoryBox.QT` est remplacé par une implémentation Rust native
...
- **Décision** : Lire le fichier `.md` de la boîte à histoires pour extraire le numéro de série matériel (`serial-XXXX`)
...
- **À revalider si** : boîte à histoires change le format de son fichier `.md`

### 2026-05-22 · Réorganisation histoires via fichier `.pi` boîte à histoires
- **Contexte** : L'ordre de lecture sur la boîte est défini par `.pi` (et `.pi.hidden`)
- **Décision** : Lire `.pi` pour afficher l'ordre réel + réécrire `.pi` ET `.pi.hidden` à chaque réordonnancement (comme le fait `StoryBox.QT`)
...
### 2026-05-22 · Dépendances Python dans ~/.synchro_boite_a_histoires/ (pas dans le bundle)
- **Contexte** : Le bundle `.app` est en lecture seule — `StoryBox.QT` et `studio-pack-generator` ne pouvaient pas être écrits dedans
- **Décision** : Télécharger/cacher les dépendances dans `~/.synchro_boite_a_histoires/` au premier transfert
...
### 2026-05-22 · `boite-bridge.py` bundlé dans les resources Tauri
- **Contexte** : `boite-bridge.py` introuvable en production dans le `.app`
- **Décision** : Déclaré dans `resources: ["../boite-bridge.py"]` dans `tauri.conf.json` ; Rust le cherche dans `Resources/_up_/`
